// Core & Auth (Phase 1)
// - Session & users (first-run Admin Setup — no default admin)
// - LocalStorage helpers
// - Theme & Language toggles
// - Minimal routing between Auth and App shell
...
(function(){
  ...
  function setLang(v){ localStorage.setItem(LANG_KEY,v); $('#langBtn').textContent = v.toUpperCase(); I18N.setLang(v); syncStaticText(); }

  function syncStaticText(){
    const L = I18N.t;
    $('#title').textContent      = L('app_title');
    $('#logoutBtn').textContent  = L('logout');
    $('#appHint').textContent    = L('app_hint');
    $('#goStore').textContent    = L('nav_store');
    $('#goSheet').textContent    = L('nav_sheet');
  }
  ...
  function renderApp(){
    ...
    $('#welcome').textContent = I18N.t('welcome', {name: session.username});
  }

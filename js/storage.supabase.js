...
(function(){
  if (!window.SUPABASE || !window.SUPABASE.url || !window.SUPABASE.anon) {
    console.error('SUPABASE config missing. Define window.SUPABASE {url, anon} before loading storage.supabase.js');
    return;
  }
  ...
  async function loadProfile(username){
    const { data, error } = await supa()
      ...
      .eq('owner', username)
      .maybeSingle();
    if (error && error.code !== 'PGRST116') { // not found is ok (PGRST116)
  ...
  async function saveProfile(username, profile){
    ...
    const { error } = await supa().from('di_profiles').upsert(row, { onConflict: 'owner' });
